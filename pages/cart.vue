<template>
    <div class="flex flex-col h-full w-full max-w-screen-xl">
                    ...
                    <tbody>
                        <tr v-for="(product, index) in products" :key="index">
                            ...
                            <td class="text-center">
                                <input class="text-center" type="number" :value="product.quantity"
                                    @change="(event) => handleQuantityChange(index, event.target.value)" />
                            </td>
                            <td>
                ...
                <div class="sticky top-4">
                    <div>
                        ...
                        <div class="flex gap-2">
                            <span class="flex-1"><input placeholder="Put in your coupoon code" /></span>
                    ...
                    <ul>
                        <li class="flex">
                            ...
                            <span class="flex-1">${{ totalExclTax() }}</span>
                        </li>
                        <li class="flex text-green-500">
                            <span class="flex-1">Discount</span>
                            <span class="flex-1">-${{ discount(totalExclTax()) }}({{ discountRate * 100 }}%)</span>
                        </li>
                        <li class="flex">
                            <span class="flex-1">VAT</span>
                            <span class="flex-1">${{ vat(totalExclTax()) }}({{ vatRate * 100 }}%)</span>
                        </li>
                        <li class="flex">
                            <span class="flex-1">Total</span>
                            <span class="flex-1">${{ totalInclTax() }}</span>
                        </li>
                    </ul>
        ...
        <div>{{ new Intl.NumberFormat('en-US', { style: 'currency', currency: 'EUR' }).format(123456.789) }}</div>
